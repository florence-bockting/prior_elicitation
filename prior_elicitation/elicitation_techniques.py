...
def _moment_elicitation(model_type, S, i, input_settings_learning, n_groups):
    ...
    if model_type == "expert":
        ...
        # sample rep_mod times from Normal distribution with resp. loc and scale
        S_normal = tfd.Normal(loc=output[f"mean_{i}"], scale=output[f"sd_{i}"]).sample(int(input_settings_learning["rep_exp"]))
        # reshape tensor from (rep_mod,1,n_gr) to (1,rep_mod,n_gr)
        output = tf.transpose(S_normal, [1,0,2])
        # broadcast output to align tensor shape with 'simulation model' tensor 
        # shape: (B,rep_exp, n_gr) or (B, rep_exp)
        if n_groups == 1:
            output = tf.broadcast_to(output, 
                                        shape=(input_settings_learning["B"],
                                            int(input_settings_learning["rep_exp"])))
        else:
            output = tf.broadcast_to(output, 
                                        shape=(input_settings_learning["B"],
                                            int(input_settings_learning["rep_exp"]),
...
class _apply_elicitation_technique(tf.keras.Model):
    ...
    def __call__(self, generated_samples, input_settings_loss, 
        ...
        # if loss_format_type: quantiles/moments
        # compute summary statistics from generated samples 
        # if loss_format: hist
        # return input
    ...
    def _f(self, samples, name_trans_loss, loss_input_type, 
        ...
        for i, S in enumerate(samples):
            ...
            if input_settings_loss["tensor_type"] == "model_only":
                output = S
