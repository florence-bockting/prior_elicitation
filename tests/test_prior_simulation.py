...
def global_dict():
    ...
    model_parameters=dict(
        mu=dict(family=tfd.Normal, 
                hyperparams_dict={
                      "mu_loc": tfd.Uniform(0.,1.),
                      "mu_scale": tfd.Uniform(0.,1.)
                      },
                param_scaling=1.
                ),
...
def test_intialize_priors_counts(global_dict):
    ...
    parameters = set(global_dict["model_parameters"].keys()).difference({"independence"})
...
def test_intialize_priors_labels(global_dict):   
    res = intialize_priors(global_dict)
    parameters = sorted(list(set(global_dict["model_parameters"].keys()).difference({"independence"})))
    
    # correct labelling of hyperparameters
    expected_hyperparam = []
    observed_hyperparam = []
    for i, param in enumerate(parameters):
        expected_hyperparam.append(global_dict["model_parameters"][param]["hyperparams_dict"].keys())
...
def test_initialize_priors_file(global_dict):
    ...
    observed_file = pd.read_pickle(global_dict["training_settings"]["output_path"]+"/init_prior.pkl")
