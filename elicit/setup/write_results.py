...
def create_output_summary(path_res, global_dict):
    ...
    if global_dict["method"] == "deep_prior":
        ...
        def method_settings():
            args_base_dist = {
                f"{key}": global_dict["model_params"]["normalizing_flow_specs"][
                    "base_distribution"
                ]["family_args"][key]
                for key in global_dict["model_params"]["normalizing_flow_specs"][
                    "base_distribution"
                ]["family_args"].keys()
            }
        ...
        def method_settings():
            family_list = [
                global_dict["model_params"]["family"][i].name
                for i in range(len(global_dict["model_params"]["family"]))
            ]
            family_dict = {
                f"{param}": family_list[i]
                for i, param in enumerate(global_dict["model_params"]["name"])
            }
            init_dict = {}
            for i in range(len(global_dict["model_params"]["family"])):
                for j, key in enumerate(
                    global_dict["model_params"]["hyperparams_dict"][i].keys()
                ):
                    init_info = global_dict["model_params"]["hyperparams_dict"][i][
                        key
                    ].parameters.copy()
                    ...
                    init_dict[
                        list(global_dict["model_params"]["hyperparams_dict"][i].keys())[
                            j
                        ]
                    ] = init_info
    ...
    if global_dict["expert_input"]["simulate_data"]:
        ...
        for key in param_list:
            true_info = global_dict["expert_input"]["simulator_specs"][
                key
            ].parameters.copy()
