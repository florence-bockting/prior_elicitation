...
def create_output_summary(path_res, global_dict):
    ...
    optimizer_dict = {}
    if type(global_dict['optimization_settings']['optimizer_specs']['learning_rate']) == float:
        optimizer_dict["init_lr"]=global_dict['optimization_settings']['optimizer_specs']['learning_rate']
    elif global_dict['optimization_settings']['optimizer_specs']['learning_rate']._keras_api_names[0] == 'keras.optimizers.schedules.CosineDecayRestarts':
    ...
    elif global_dict['optimization_settings']['optimizer_specs']['learning_rate']._keras_api_names[0] == 'keras.optimizers.schedules.ExponentialDecay':
        optimizer_dict["lr_scheduler"]=global_dict['optimization_settings']['optimizer_specs']['learning_rate']._keras_api_names[0]
        optimizer_dict["init_lr"]=global_dict['optimization_settings']['optimizer_specs']['learning_rate'].initial_learning_rate
    ...
    if global_dict['method'] == "deep_prior":
        def method_settings():
            args_base_dist = {f"{key}": global_dict['model_params']['normalizing_flow_specs']['base_distribution']['family_args'][key] for key in global_dict['model_params']['normalizing_flow_specs']['base_distribution']['family_args'].keys()}
            ...
            f"base distribution family={inspect.getfullargspec(global_dict['model_params']['normalizing_flow_specs']['base_distribution']['family'])[3][-1]}\n"+
            f"base distribution arguments={args_base_dist}\n")
    else:
        def method_settings():
    ...
    if global_dict["expert_input"]["simulate_data"]:
        param_list = global_dict["expert_input"]['simulator_specs'].keys() 
        true_info2 = {}
        for key in param_list:
            true_info = global_dict["expert_input"]['simulator_specs'][key].parameters.copy()
    ...
    output_summary = str(
        ...
        f"model parameter scaling={global_dict['model_params']['scaling_value']}\n"+
        f"model parameter independent={global_dict['param_independence']['independent']}\n"+
        f"independent loss scaling factor={global_dict['param_independence']['loss-scaling']}\n"+
        "\nExpert info"+
        "\n---------------- \n"+
        f"{expert_info}\n"+
        method_settings()+
