...
def target(name: str, 
    ...
    assert elicitation_method in ["quantiles","quantiles_mean", "histogram", "moments","identity"], f"The elicitation method {elicitation_method} is not implemented"
...
def expert(data: str or None,
    ...
    if simulate_data:
        assert simulator_specs is not None, "if expert simulation is true simulator specifications need to be provided"
...
def loss(loss_function: str or callable = "mmd-energy", 
    ...
    if loss_weighting is not None:
        assert loss_weighting["method"] in ["dwa", "custom"], "Currently only the following loss-weighting methods are implemented 'dwa'."
...
def prior_elicitation(
    ...
    global_dict = create_global_dict(
        method, sim_id, epochs, B, rep, seed, burnin, param_independence, 
        model_params, expert_input, generative_model, target_quantities, 
        loss_function, optimization_settings, output_path, print_info, view_ep)
