...
def param(
    ...
    if hyperparams_dict is not None:
        hyppar_dict = {
            key: value
            for key, value in zip(hyperparams_dict.keys(), hyperparams_dict.values())
        }
    else:
        hyppar_dict = None
...
def model(
    ...
    softmax_gumble_specs: dict or None = {"temperature": 1.0, "upper_threshold": None},
) -> dict:
    """
    ...
    if softmax_gumble_specs is not None:
        softmax_gumble_specs_default = {"temperature": 1.0, "upper_threshold": None}
    ...
    if discrete_likelihood:
        assert (
            "upper_threshold" in softmax_gumble_specs
        ), "The upper_threshold argument in the softmax-gumble specifications is None."
        if "temperature" not in softmax_gumble_specs:
            warnings.warn(
                f"The Softmax-Gumble method with default temperature: {softmax_gumble_specs_default['temperature']} is used."
            )
    # get model arguments
    get_model_args = set(inspect.getfullargspec(model())[0]).difference(
        {"self", "prior_samples"}
    )
    # check that all model arguments have been passed as input in the model_args section
    if additional_model_args is not None:
        set_args = set(additional_model_args.keys())
        set_args.add("ground_truth")
        difference_set = get_model_args.difference(set_args)
        assert (
            len(difference_set) == 0
        ), f"model arguments and specified arguments are not identical. Check the following argument(s): {difference_set}"
...
def target(
    ...
    custom_elicitation_function: dict or None = None,
) -> dict:
    """
    ...
    def check_custom_func(custom_function, func_type):
        if custom_function is not None:
            ...
            assert "function" in list(
                custom_function.keys()
            ), f"custom_{func_type}_function must be a dictionary with required key 'function' and optional key 'additional_args'"
            default_custom_function = {"function": None, "additional_args": None}
            ...
            try:
                ...
                if default_custom_function["additional_args"] is not None:
                    assert (
                        type(default_custom_function["additional_args"]) is dict
                    ), "additional_args must be a dictionary keys are the name of the argument and value the corresponding value of the argument to be passed"
    ...
    assert loss_components in [
        "by-group",
        "all",
        "by-stats",
    ], "Currently only available values for loss_components are 'all', 'by-stats' or 'by-group'."
    ...
    assert elicitation_method in [
        ...
        "moments",
        "identity",
    ], f"The elicitation method {elicitation_method} is not implemented"
    # if elicitation method 'quantiles' is used, quantiles_specs has to be defined
    if elicitation_method == "quantiles":
        assert (
            quantiles_specs is not None
        ), "quantiles_specs has to be defined for elicitation method 'quantiles'"
    if elicitation_method == "moments":
        assert (
            moments_specs is not None
        ), "moments_specs has to be defined for elicitation method 'moments'"
...
def expert(
    ...
    assert not (
        data is None and simulate_data is False
    ), "either a path to expert data has to be specified or simulation of expert data has to be true."
    # if expert simulator is true, simulator specifications have to be given
    if simulate_data:
        assert (
            simulator_specs is not None
        ), "if expert simulation is true simulator specifications need to be provided"
...
def loss(
    ...
    if type(loss_function) is str:
        assert loss_function in [
            "mmd-energy"
        ], "Currently only the following loss functions are implemented 'mmd-energy'."
    ...
    if loss_weighting is not None:
        assert loss_weighting["method"] in [
            "dwa",
            "custom",
        ], "Currently only the following loss-weighting methods are implemented 'dwa'."

    loss_dict = {"loss_function": call_loss_function, "loss_weighting": loss_weighting}
    return loss_dict
...
def prior_elicitation(
    ...
    param_independence : bool
        whether model parameters should be treated as independent. If True, correlation between model parameters is
        incorporated in the loss function.
    model_params : callable
