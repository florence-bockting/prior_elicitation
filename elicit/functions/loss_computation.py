...
def compute_loss_components(elicited_statistics, global_dict, expert):
    ...
    # for later checking in if statement
    ...
    if global_dict["param_independence"]["independent"]:
        if "correlation" in name_elicits_copy:
            ...
            for j in range(loss_component.shape[-1]):
                correl_loss_component = loss_component[:,j]
                if tf.rank(correl_loss_component) == 1:
                    correl_loss_component = tf.expand_dims(correl_loss_component, axis = -1)
...
def compute_discrepancy(loss_components_expert, loss_components_training,
    ...
    if global_dict["param_independence"]["independent"]:
        keys_loss_components = [x for x in keys_loss_components if not x.startswith("correlation_loss")]
    # compute discrepancy 
    for name in keys_loss_components:
        # broadcast expert loss to training-shape
        loss_comp_expert = tf.broadcast_to(loss_components_expert[name], 
                                           shape=(loss_components_training[name].shape[0],
    ...
    if global_dict["param_independence"]["independent"]:
        keys_loss_components = [x for x in list(loss_components_training.keys()) if x.startswith("correlation_loss")]
        for key_loss in keys_loss_components:
            loss_per_component.append(norm_diff(loss_components_training[key_loss])*global_dict["param_independence"]["loss-scaling"])      
