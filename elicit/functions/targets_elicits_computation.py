...
def computation_elicited_statistics(target_quantities, ground_truth, global_dict):
    ...
    for i, (target, elicit) in enumerate(zip(name_targets, name_elicits)):
        ...
        assert elicit in set(["quantiles","quantiles_mean", "histogram", "moments","identity"]), "Name error of elicitation techniques. Currently supported elicitation techniques are quantiles, histogram, moments."
        ...
        if elicit == "histogram":
            ...
            target_hist = tfp.stats.percentile(target_quantities[target], q = quantiles_hist, axis = -1)        
        ...
        if elicit == "quantiles":
            ...
            if tf.rank(quan) == 3:
                quan_reshaped = tf.reshape(quan, (quan.shape[0],quan.shape[1]*quan.shape[2]))
            if tf.rank(quan) == 2:
                quan_reshaped = quan
            computed_quantiles = tfp.stats.percentile(quan_reshaped, q = quantiles, axis = -1)
            # else:
        ...
        if elicit == "quantiles_mean":
            quantiles = sub_global_dict["quantiles_specs"][i]
            # compute quantiles
            computed_quantiles = tfp.stats.percentile(target_quantities[target], q = quantiles, axis = 2)
            # average over rep simulations from priors
            computed_quantiles_avg = tf.reduce_mean(computed_quantiles, 2)
            # bring quantiles to the last dimension
            elicited_statistic = tf.einsum("ij...->ji...", computed_quantiles_avg)
