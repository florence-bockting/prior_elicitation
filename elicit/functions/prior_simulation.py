...
def sample_from_priors(initialized_priors, ground_truth, global_dict):
    ...
    if method == "deep_prior" and not ground_truth:
        ...
        base_dist_family = global_dict["model_params"]["normalizing_flow_specs"]["base_distribution"]["family"]
        base_dist_args = global_dict["model_params"]["normalizing_flow_specs"]["base_distribution"]["family_args"]
        # get params of distribution family
        family_params = set(inspect.getfullargspec(base_dist_family)[0]).difference(set(["self",'validate_args','allow_nan_stats', 'name']))
        # check whether arguments are named correctly
        try:
            set(base_dist_args.keys()).issubset(family_params)
        except:
            print(f"family_args of base distribution must match with parameter names of function provided in family. Got {set(base_dist_args.keys())} but require {family_params}")
        ...
        try:
            ...
            print(f"shape of base distribution does not match with number of model parameters. Got {list(base_dist.event_shape)[0]} but require {no_param}")
