...
def intialize_priors(global_dict):
    ...
    if global_dict["training_settings"]["method"] == "parametric_prior":
        ...
        for model_param in set(global_dict["model_parameters"].keys()).symmetric_difference(set(["independence", "no_params"])):
...
def sample_from_priors(initialized_priors, ground_truth, global_dict):
    ...
    if (global_dict["training_settings"]["method"] == "parametric_prior") and (not ground_truth):
        ...
        priors = []
        for i, param in enumerate(set(global_dict["model_parameters"].keys()).symmetric_difference(set(["independence","no_params"]))):
