...
def norm_diff(correlation_training, axis=None, ord="euclidean"):
    ...
    """
    difference = tf.subtract(tf.zeros(correlation_training.shape), correlation_training)
...
class MmdEnergy(tf.Module):
    ...
    def squared_distances(self, loss_component_expert, loss_component_training):
        ...
        distance_expert = tf.expand_dims(
            tf.math.reduce_sum(
                tf.multiply(loss_component_expert, loss_component_expert), axis=-1
            ),
            axis=2,
        )
        # (B, 1, M)
        distance_training = tf.expand_dims(
            tf.math.reduce_sum(
                tf.multiply(loss_component_training, loss_component_training), axis=-1
            ),
            axis=1,
        )
        # (B, N, M)
        distance_expert_training = tf.matmul(
            loss_component_expert, tf.transpose(loss_component_training, perm=(0, 2, 1))
        )
        # compute sq. distance
        squared_distance = (
    ...
    def kernel_loss(
        self, loss_component_expert, loss_component_training, a, b, dim_expert
    ):
