...
time_total = [tf.reduce_sum(
    pd.read_pickle(path_sim_res+f"/binomial_{i}/final_results.pkl")["time_epoch"]
    ) for i in range(30)]

# compute mean and sd across all 30 replications
time_res = dict(
    mean_time = tf.reduce_mean(time_total),
    sd_time = tf.math.reduce_std(time_total)
    )
...
# path to simulation results
path_sim_res = f"elicit/simulations/LiDO_cluster/simulation_results/normal_{scenario}"

# get total training time per replication
time_total = [tf.reduce_sum(
    pd.read_pickle(path_sim_res+f"/normal_{scenario}_{i}/final_results.pkl")["time_epoch"]
    ) for i in range(30)]
