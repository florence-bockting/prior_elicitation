...
def initialization(eliobj, cols: int = 4, **kwargs):
    """
    ...
    **kwargs : any, optional
    ...
    low = tf.subtract(eliobj.initializer["distribution"]["mean"],
                      eliobj.initializer["distribution"]["radius"])
    high = tf.add(eliobj.initializer["distribution"]["mean"],
                  eliobj.initializer["distribution"]["radius"])
    ...
    fig, axs = plt.subplots(rows, cols, constrained_layout=True, sharey=True,
                            **kwargs)
    if rows == 1:
        for c,hyp, lo, hi in zip(tf.range(cols), eliobj.results["init_matrix"],
                                 low, high):
            sns.ecdfplot(tf.squeeze(eliobj.results["init_matrix"][hyp]),
                         ax=axs[c], color="black", lw=2)
        ...
        for (r,c),hyp, lo, hi in zip(
                ...
                eliobj.results["init_matrix"], low, high):
            sns.ecdfplot(tf.squeeze(eliobj.results["init_matrix"][hyp]),
                         ax=axs[r,c], color="black", lw=2)
            axs[r,c].set_title(f"{hyp}", fontsize="small") 
            axs[r,c].axline((lo,0),(hi,1), color="grey", linestyle="dashed",
                            lw=1)
...
def loss(eliobj, **kwargs):
    ...
    **kwargs : any, optional
    ...
    [axs[i].set_title(t, fontsize="small") for i,t in enumerate(
...
def hyperparameter(eliobj, cols: int=4, span: int=30, **kwargs):
    """
    ...
    if rows == 1:
        ...
        for (r,c),hyp in zip(
            ...
            axs[r,c].axhline(avg_hyp.numpy(), color="darkgrey",
                             linestyle="dotted")
            axs[r,c].plot(eliobj.history["hyperparameter"][hyp], color="black",
                          lw=2)
            axs[r,c].set_title(f"{hyp}", fontsize="small") 
            axs[r,c].tick_params(axis="y", labelsize="x-small")
            axs[r,c].tick_params(axis="x", labelsize="x-small")
...
def priors(eliobj, constraints: dict, **kwargs):
    ...
    **kwargs : any, optional
    ...
    priors = tf.reshape(eliobj.results["prior_samples"],
                        (B*n_samples, n_params))
    # get parameter names
    name_params = [eliobj.parameters[i]["name"] for i in 
                   range(len(eliobj.parameters))]
...
def elicits(eliobj, cols: int=4, **kwargs):
    """
