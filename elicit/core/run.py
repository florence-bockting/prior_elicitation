...
class PrepareDag():
    ...
    #     # set seed for sampling expert data
    #     tf.random.set_seed(self.seed)
        

    def one_forward_simulation(self, prior_model, global_dict, ground_truth=False):
        """
    ...
    def load_expert_data(self, global_dict, path_to_expert_data=None):
        """
        ...
        if global_dict["expert_data"]["from_ground_truth"]:
            ...
            expert_data = self.one_forward_simulation(
    ...
    def burnin_phase(self, 
        ...
        for i in range(global_dict["training_settings"]["warmup_initializations"]):
            ...
            training_elicited_statistics = self.one_forward_simulation(
                prior_model, global_dict
            )
            # compute loss for each set of initial values
            weighted_total_loss = self.compute_loss(
...
def prior_elicitation(
    ...
    #%% HELPER VALUES
    num_params = len(set(model_parameters.keys()).symmetric_difference(set(["independence"])))
    ...
    if training_settings["method"] == "parametric_prior":
        for param_name in set(model_parameters.keys()).symmetric_difference(set(["independence"])):
    ...
    # get expert data
    expert_elicited_statistics = dag.load_expert_data(global_dict)
    ...
    if global_dict["training_settings"]["warmup_initializations"] is None:
        ...
        loss_list, init_prior = dag.burnin_phase(
            expert_elicited_statistics, 
            dag.one_forward_simulation, 
            dag.compute_loss, 
            global_dict
        )
