...
def plot_results_overview_param(path, file, title):
    ...
    hyperparams = tf.stack([final_res["hyperparameter"][key] for key in final_res["hyperparameter"].keys()], -1)
    
    hyperparam_loc =  tf.gather(hyperparams, [0,2,4,6], axis=1)
    hyperparam_scale =  tf.gather(tf.exp(hyperparams), [1,3,5,7], axis=1)
    ...
    [fig0[1,0].axhline(expert_loc[i], linestyle = "dashed", color = "black") for i in range(expert_loc.shape[0])]
    [fig0[1,1].axhline(expert_scale[i], linestyle = "dashed", color = "black") for i in range(expert_loc.shape[0])]
    [fig0[1,0].plot(range(len(total_loss)), hyperparam_loc[:,i], lw =3) for i in range(hyperparam_loc.shape[1])]
    [fig0[1,1].plot(range(len(total_loss)), hyperparam_scale[:,i], lw =3, label = fr"$\lambda_{i}$") for i in range(hyperparam_scale.shape[1])]
    fig0[1,1].legend(loc="upper right", handlelength=0.4, fontsize="small", ncol=4)
