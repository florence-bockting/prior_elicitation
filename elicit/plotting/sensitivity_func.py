...
def plot_prior_binom(sim_path, expert_path, file, seed=None, save_fig=True):
    ...
    subfigs[1].suptitle(
        r"$\mathbf{(b)}$" + f" Joint prior distribution (seed: {int(seed)})",
        ha="left",
        x=0.01,
        fontsize="medium",
    )
    subfigs[0].suptitle(
        r"$\mathbf{(a)}$" + f" Convergence diagnostics (seed: {int(seed)})",
        ha="left",
        x=0.01,
        fontsize="medium",
    )
...
def plot_prior_binom_independent(path_sim_res, seed):
    final_res = pd.read_pickle(
        path_sim_res + "/final_results.pkl")["hyperparameter"]
    final_hyps = dict()
    ...
    axs[0].plot(
        xrge, tfd.Normal(0.1, 0.1).prob(xrge), color="black",
        linestyle="dashed"
    )
    axs[0].plot(
        xrge,
        tfd.Normal(final_hyps["mu0"], final_hyps["sigma0"]).prob(xrge),
        color="#26bfbf",
        lw=2,
    )
    axs[1].plot(
        xrge2,
        tfd.Normal(-0.1, 0.3).prob(xrge2),
        color="black",
        linestyle="dashed",
        label="true",
    )
    axs[1].plot(
        xrge2,
        tfd.Normal(final_hyps["mu1"], final_hyps["sigma1"]).prob(xrge2),
        color="#26bfbf",
        lw=2,
        label="learned",
    )
    ...
    plt.suptitle(f"learned prior distributions, seed={seed}", ha="left",
                 x=0.03)
    plt.show()
...
def plot_loss_binom_independent(sim_path, expert_path, file=None,
                                save_fig=True):
    if file is None:
        final_res = pd.read_pickle(sim_path + "/final_results.pkl")
    else:
    ...
    total_loss = tf.stack(final_res["loss"], -1)
    component_loss = tf.stack(final_res["loss_component"], -1)
    marginal_means = tf.stack(
        [
            pd.read_pickle(
                sim_path + "/final_results.pkl")["hyperparameter"][m]
            for m in ["mu0", "mu1"]
        ],
        -1,
    )
    marginal_sds = tf.stack(
        [
            tf.exp(pd.read_pickle(
                sim_path + "/final_results.pkl")["hyperparameter"][m])
            for m in ["log_sigma0", "log_sigma1"]
        ],
        -1,
    )
    expert_res = pd.read_pickle(expert_path + "/prior_samples.pkl")
    ...
    [
        axs[1, i].set_title(t, fontsize="small")
        for i, t in enumerate(["location hyperparam.", "scale hyperparam."])
    ]
    [axs[1, i].set_xlabel("epochs", fontsize="x-small") for i in range(2)]


def plot_elicited_stats_binom(
    prior_expert,
    path_expert,
    path_sim,
    elicit_res_agg,
...
def plot_elicited_stats_binom_independent(
    prior_expert,
    path_expert,
    path_sim,
    elicit_res_agg,
...
def plot_elicited_stats(
    ...
    exp_r2 = tf.exp(
        pd.read_pickle(
            expert_path + "/elicited_statistics.pkl")["quantiles_logR2"]
    )
