...
def binomial_convergence(sim_path, expert_path, file, save_fig=True):
    ...
    [axs0[1,0].axhline(exp_means[i], color="black", linestyle="dashed", lw=1)
     for i in range(2)] 
    [axs0[1,1].axhline(exp_sds[i], color="black", linestyle="dashed", lw=1)
     for i in range(2)] 
    [axs0[1,0].plot(
        range(marginal_means.shape[0]), marginal_means[:,i],
        label=rf"m$(\theta_{i})$", color=col_betas[i]) for i in range(2)]
    [axs0[1,1].plot(
        range(marginal_sds.shape[0]), marginal_sds[:,i],
        label=rf"sd$(\theta_{i})$", color=col_betas[i]) for i in range(2)]
    axs0[1,0].legend(handlelength=0.5,ncol=2, fontsize="x-small",
                     frameon=False, loc="center right")
    axs0[1,1].legend(handlelength=0.5,ncol=2, fontsize="x-small",
                     frameon=False, loc="upper right")
    [axs0[j,i].spines[['right', 'top']].set_visible(False)
     for j,i in product([0,1],[0,1])]
    [axs0[j,i].yaxis.set_tick_params(labelsize=7)
     for j,i in product([0,1],[0,1])]
    ...
    sns.kdeplot(exp_priors[0,:,0], color=col_exp, ax=axs1[0,0], lw=2,
                label="true", zorder=1)
    sns.kdeplot(exp_priors[0,:,1], color=col_exp, ax=axs1[1,1], lw=2,
                label="true", zorder=1)
    sns.kdeplot(mod_priors[0,:,1], lw=3, color=col_mod, ax=axs1[1,1],
                 zorder=0)
    sns.kdeplot(mod_priors[0,:,0], lw=3, color=col_mod, label="learned",
                 ax=axs1[0,0], zorder=0)
    ...
    axs1[1,0].text(
        0.5, 0.3, f"corr:\n{cor([0,1], mod_priors[0,:,:]):.2f}", color="black",
        fontsize="small", ha="center")
    [axs1[j,i].xaxis.set_tick_params(labelsize=7)
     for i,j in product([0,1],[0,1])]
    [axs1[j,i].yaxis.set_tick_params(labelsize=7)
     for i,j in product([0,1],[0,1])]
    axs1[0,0].legend(handlelength=0.3,ncol=1, fontsize="x-small",
    ...
    if save_fig:
        plt.savefig(
            "elicit/simulations/LiDO_cluster/sim_results/deep_prior/graphics/convergence_binom.png",
            dpi=300)
    else:
        plt.show()
...
def binomial_diagnostics(sim_path, expert_path, file, save_fig=True):
    ...
    [axs[1,0].plot(range(marginal_means.shape[0]), marginal_means[:,i],
                   label=rf"m$(\theta_{i})$", color=col_betas[i])
     for i in range(2)]
    [axs[1,1].plot(range(marginal_sds.shape[0]), marginal_sds[:,i],
                   label=rf"sd$(\theta_{i})$", color=col_betas[i])
     for i in range(2)]
    ...
    [axs[1,i].set_title(t, fontsize="small") for i,t in
     enumerate(["marginal means","marginal std. dev."])]
    [axs[1,i].set_xlabel("epochs", fontsize="x-small") for i in range(2)]
    if save_fig:
        plt.savefig(
            "elicit/simulations/LiDO_cluster/sim_results/deep_prior/graphics/diagnostics_binom.png",
            dpi=300)
    else:
        plt.show()
...
def normals_convergence(sim_path, expert_path, file, model, save_fig=True):
    ...
    exp_priors = pd.read_pickle(expert_path+"/model_simulations.pkl")["prior_samples"]
    mod_priors = pd.read_pickle(sim_path+file+"/model_simulations.pkl")["prior_samples"]
    exp_means = tf.reduce_mean(exp_priors, (0,1))
    ...
    axs0[0,0].plot(range(total_loss.shape[0]), total_loss, color="black")
    [axs0[0,1].plot(range(component_loss.shape[1]), component_loss[i,:], color=col_losses[i,:]) for i in range(component_loss.shape[0])]
   # [axs0[0,i].set_yscale('log', base=2) for i in range(2)]
    [axs0[0,i].set_title(t, fontsize="small") for i,t in enumerate(["total loss","loss components"])]   
    [axs0[1,0].axhline(exp_means[i], color="black", linestyle="dashed", lw=1) for i in range(4)] 
    [axs0[1,1].axhline(exp_sds[i], color="black", linestyle="dashed", lw=1) for i in range(4)] 
    [axs0[1,0].plot(range(marginal_means.shape[0]), marginal_means[:,i], label=rf"m$(\theta_{i})$",
                   color=col_betas[i]) for i in range(4)]
    [axs0[1,1].plot(range(marginal_sds.shape[0]), marginal_sds[:,i], label=rf"sd$(\theta_{i})$",
                   color=col_betas[i]) for i in range(4)]
    axs0[1,1].legend(handlelength=0.2,ncol=2, fontsize="x-small", frameon=False,
                     loc=(0.5,0.5), columnspacing=0.1, handletextpad=0.2)
    axs0[1,0].legend(handlelength=0.2,ncol=4, fontsize="x-small", frameon=False,
                      loc=(0.,0.8), columnspacing=0.1, handletextpad=0.2)
    [axs0[j,i].spines[['right', 'top']].set_visible(False) for j,i in product([0,1],[0,1])]
    [axs0[j,i].yaxis.set_tick_params(labelsize=7) for j,i in product([0,1],[0,1])]
    [axs0[1,i].xaxis.set_tick_params(labelsize=7) for i in range(2)]
    [axs0[1,i].set_title(t, fontsize="small") for i,t in enumerate(["marginal means","marginal std. dev."])]  
    [axs0[1,i].set_xlabel("epochs", fontsize="x-small") for i in range(2)]
    axs0[1,0].set_ylim(-3, 15)
    axs0[1,1].set_ylim(-1, 15)


    sns.kdeplot(exp_priors[0,:,3], color = col_exp, ax=axs1[3,3], lw=2, label="true")
    sns.kdeplot(mod_priors[0,:,3], lw = 1, color = col_mod, 
                     ax=axs1[3,3], label="learned")
    for i in range(4):
        sns.kdeplot(exp_priors[0,:,i], color = col_exp, ax=axs1[i,i], lw=2)
        sns.kdeplot(mod_priors[0,:,i], lw = 2, color = col_mod, 
                         ax=axs1[i,i])
    ...
    for i,j in zip([0,0,0,1,1,2],[1,2,3,2,3,3]):
        ...
        axs1[j,i].spines[['right', 'top', "left","bottom"]].set_visible(False) 
        axs1[j,i].text(0.5, 0.3, f"corr:\n{cor([j,i], mod_priors[0,:,:]):.2f}", color="black", 
                      fontsize="small", ha="center")
    [axs1[i,j].spines[['right', 'top']].set_visible(False) for i,j in product(range(4),range(4))]
    [axs1[0,i].set_title(fr"$\beta_{i}$", fontsize="small") for i in range(3)]
    axs1[0,3].set_title("$\sigma$", fontsize="small")
    axs1[3,3].legend(handlelength=0.5,ncol=1, fontsize="x-small", frameon=False,
                     loc=(0.4,0.5))

    subfigs[1].suptitle(r"$\mathbf{(b)}$"+f" Joint prior distribution (seed: {file.split('_')[-1]})", ha="left", 
                        x = 0.01, fontsize="medium")
    subfigs[0].suptitle(r"$\mathbf{(a)}$"+f" Convergence diagnostics (seed: {file.split('_')[-1]})", ha="left", 
                        x = 0.01, fontsize="medium")
    if save_fig:
        plt.savefig(f"elicit/simulations/LiDO_cluster/sim_results/deep_prior/graphics/convergence_{model}.png", 
                    dpi=300)
    else:
        plt.show()
