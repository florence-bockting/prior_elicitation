...
for i in range(len(all_files)):
    if len(pd.read_pickle(
        ...
        means_res=tf.stack(
            pd.read_pickle(path_sim+f"/{all_files[i]}"+"/final_results.pkl"
...
scenario="correlated"
path_sim = f"elicit/simulations/LiDO_cluster/sim_results/deep_prior/normal_{scenario}2"
...
elicits_r2_list = []
for i in range(len(all_files)):
    if len(pd.read_pickle(path_sim+f"/{all_files[i]}"+"/final_results.pkl")["loss"]) == 1500:
        prior_res = pd.read_pickle(path_sim+f"/{all_files[i]}"+"/model_simulations.pkl")["prior_samples"]
        elicits_gr1 = pd.read_pickle(path_sim+f"/{all_files[i]}"+"/elicited_statistics.pkl")["quantiles_group1"]
        elicits_gr2 = pd.read_pickle(path_sim+f"/{all_files[i]}"+"/elicited_statistics.pkl")["quantiles_group2"]
...
prior_expert = pd.read_pickle(f"elicit/simulations/LiDO_cluster/experts/deep_{scenario}_normal/prior_samples.pkl")
...
normals_convergence(path_sim, path_expert, f"/normal_{scenario}_6", model=scenario,
                    save_fig=False)
