...
def plot_conv_diagnostics(path_sim, start, end, last_vals,scenario=None,
                          max_seeds=4, factor=100, num_seeds=30):
    ...
    for i in range(num_seeds):
        res_loss = tf.stack(
            pd.read_pickle(path_sim+f"/{all_files[i]}"+"/final_results.pkl")["loss"]
            )
    ...
    subfig0.plot(range(num_seeds-max_seeds,num_seeds), res_highest["slope"], "o", color="#21284f")
    ...
    for i, c in enumerate(["#2e5c2c"]+["#21284f"]*4):
        ...
        subfig1[i].plot(range(last_vals-100,last_vals), single_losses[i][-100:], 
                        color=c)
        subfig1[i].plot([last_vals-100, last_vals], 
                        [np.mean(single_losses[i][-105:-95]),
                         (res_single.iloc[i]["slope_o"]/100)*100+np.mean(single_losses[i][-105:-95])],
                        "-", color="red")
        # subfig1[i].axline(xy1=(last_vals-50,np.mean(single_losses[i][-50:])), 
        #                   slope=res_single.iloc[i]["slope_o"]/100,
        #                   color="darkred", linestyle="dashed", lw=1.5)
        subfig1[i].set_title(f"seed: {res_single['seed'].iloc[i]}", 
                             fontsize="medium")
        subfig1[i].set_xticks(np.linspace(0,last_vals,3), 
