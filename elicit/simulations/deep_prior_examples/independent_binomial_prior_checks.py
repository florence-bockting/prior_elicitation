...
for mu0 in mu0_seq:
    run_prior_checks(
        seed, "results/binomial_independent_sensitivity/vary_mu0", mu0, 0.1, -0.1, 0.3
    )

for sigma0 in sigma0_seq:
    run_prior_checks(
        seed,
        "results/binomial_independent_sensitivity/vary_sigma0",
        0.1,
        sigma0,
        -0.1,
        0.3,
...
for mu1 in mu1_seq:
    run_prior_checks(
        seed, "results/binomial_independent_sensitivity/vary_mu1", 0.1, 0.1, mu1, 0.3
    )

for sigma1 in sigma1_seq:
    run_prior_checks(
        seed,
        "results/binomial_independent_sensitivity/vary_sigma1",
        0.1,
        0.1,
        -0.1,
        sigma1,
...
for vary in ["vary_mu0", "vary_sigma0", "vary_mu1", "vary_sigma1"]:
    path = "elicit/results/binomial_independent_sensitivity/" + vary + "/deep_prior"
...
for l, (k, xseq) in enumerate(
    ...
    for j, elicit in enumerate(["X0", "X1"]):
        for i, col in list(
            enumerate(["#21284f", "#00537b", "#007d87", "#00ac79", "#83cf4a"])
        ):
            axs[l, j].plot(
