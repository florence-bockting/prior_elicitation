...
def load_design_matrix_m1(n_group):
...
def load_design_matrix_m2(N):
    
    # construct design matrix with a 2-level factor
    df =  tf.cast(pa.dmatrix("a", pa.demo_data("a", nlevels=2, min_rows=N), 
                    return_type="dataframe"), dtype=tf.float32)
    # continuous predictor
    X_std = tfd.Normal(0.,1.).quantile([0.05, 0.25, 0.75, 0.95])
    # final design matrix
    d_final = tf.concat([df, tf.expand_dims(X_std,-1)],-1)
    return d_final

def load_design_matrix_m22(scaling, selected_obs):
    # load dataset from repo
    url = "https://github.com/bayes-rules/bayesrules/blob/404fbdbae2957976820f9249e9cc663a72141463/data-raw/equality_index/equality_index.csv?raw=true"
    df = pd.read_csv(url)
    # exclude california from analysis as extreme outlier
    df_filtered = df.loc[df["state"] != "california"]
    # use only gop and dem
    df_filtered = df.loc[df["historical"] != "swing"]
    # select predictors
    df_prep = df_filtered.loc[:, ["historical", "percent_urban"]]
    # reorder historical predictor
    df_reordered = df_prep.sort_values(["historical", "percent_urban"])
...
def load_design_matrix_m21(N, selected_obs):
    # construct design matrix with a 3-level factor
    df =  pa.demo_data("x", "y", "z", min_rows=N)
    # save in correct format
    d_final = tf.cast(pd.DataFrame(df), dtype = tf.float32)
    if selected_obs is not None:
        # select only relevant observations
        d_final = tf.gather(d_final, selected_obs, axis = 0)
    return d_final
